...
#gets the values given on parameters, only if the number of parameters are equal to 4
my $fnFeature = $ARGV[0];
my $fnPrediction = $ARGV[1];
my $fnResult = $ARGV[2];
my $flag = $ARGV[3]; #the flag, should be the values 1 or 0
if($flag != 1 && $flag != 0)
{
	print "Invalid command line.\n";
	print "Usage: perl Eval.pl argv[1] argv[2] argv[3] argv[4]\n";
...
#call subroutine ReadInputFiles
my %hsQueryDocLabelScore = ReadInputFiles($fnFeature, $fnPrediction);
#@print $hsQueryDocLabelScore;




#call hsQueryEval evaluator
