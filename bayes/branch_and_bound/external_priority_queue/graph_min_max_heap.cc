...
GraphMinMaxHeapNode::GraphMinMaxHeapNode(const Graph& key,
                                         GraphMinMaxHeapNode* parent)
    : key_(new Graph(key)), parent_(parent) {
}

GraphMinMaxHeap::GraphMinMaxHeap() : size_(0) {
}

static void Swap(GraphMinMaxHeapNode* a, GraphMinMaxHeapNode* b) {
  auto* tmp = a->mutable_key().release();
  a->mutable_key().reset(b->mutable_key().release());
  b->mutable_key().reset(tmp);
}

void GraphMinMaxHeap::push(const Graph& key) {
  size_++;
  if (size_ == 1) {
    root_.reset(new GraphMinMaxHeapNode(key, NULL));
    return;
  }
  auto* node = GetNode(size_ >> 1);
  if (node->left() == NULL) {
    node->mutable_left().reset(new GraphMinMaxHeapNode(key, node));
    node = node->left();
  } else {
    node->mutable_right().reset(new GraphMinMaxHeapNode(key, node));
    node = node->right();
  }
  GraphMinMaxHeapify(node);
}

const Graph& GraphMinMaxHeap::max() const {
  Graph* max = NULL;
  if (size_ <= 1) {
    max = root_->key().get();
  } else if (size_ == 2) {
    max = root_->left()->key().get();
  } else if (root_->left()->score() > root_->right()->score()) {
    max = root_->left()->key().get();
  }
  else {
    max = root_->right()->key().get();
  }
  return *max;
}

const Graph& GraphMinMaxHeap::min() const {
  return *(root_->key().get());
}

void GraphMinMaxHeap::pop_max() {
  if (size_ <= 1) {
    size_ = 0;
    root_.reset();
    return;
...
bool GraphMinMaxHeap::empty() const {
  return size_ == 0;
}

void GraphMinMaxHeapNode::Print(std::string left_padding) const {
...
void GraphMinMaxHeap::Print() const {
  if (root_ == NULL) {
    std::cout << "||" << std::endl;
    return;
  }
...
void GraphMinMaxHeap::GraphMinMaxHeapify(GraphMinMaxHeapNode* node) {
...
static GraphMinMaxHeapNode* Grandparent(GraphMinMaxHeapNode* node) {
...
void GraphMinMaxHeap::MaxBubbleUp(GraphMinMaxHeapNode* node) {
  if (node->parent() == NULL) {
    return;
  }
...
void GraphMinMaxHeap::MinBubbleUp(GraphMinMaxHeapNode* node) {
  if (node->parent() == NULL) {
    return;
  }
...
static bool HasGrandchildren(GraphMinMaxHeapNode* node) {
  return (node != NULL && node->left() != NULL && node->left()->left() != NULL);
}

static GraphMinMaxHeapNode* CompareNodes(GraphMinMaxHeapNode* left,
                                         GraphMinMaxHeapNode* right,
                                         bool want_max) {
  if (right == NULL) {
    return left;
  }
...
static GraphMinMaxHeapNode* Grandchild(GraphMinMaxHeapNode* node,
...
void GraphMinMaxHeap::MaxSiftDown(GraphMinMaxHeapNode* node) {
  if (node->left() != NULL) {
...
void GraphMinMaxHeap::MinSiftDown(GraphMinMaxHeapNode* node) {
  if (node->left() != NULL) {
    if (node->left()->score() < node->score()) {
      Swap(node->left(), node);
    }
...
GraphMinMaxHeapNode* GraphMinMaxHeap::GetNode(unsigned node_id) const {
  if (node_id <= 1) {
    return root_.get();
  }
