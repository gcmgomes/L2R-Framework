...
SimpleMinMaxHeapNode::SimpleMinMaxHeapNode(int key,
                                           SimpleMinMaxHeapNode* parent)
    : key_(key), parent_(parent) {
}

SimpleMinMaxHeap::SimpleMinMaxHeap() : size_(0) {
}

static void Swap(SimpleMinMaxHeapNode* a, SimpleMinMaxHeapNode* b) {
  int tmp = a->key();
  a->mutable_key() = b->key();
  b->mutable_key() = tmp;
}

void SimpleMinMaxHeap::push(int key) {
  size_++;
  if (size_ == 1) {
    root_.reset(new SimpleMinMaxHeapNode(key, NULL));
    return;
  }
  auto* node = GetNode(size_ >> 1);
  if (node->left() == NULL) {
    node->mutable_left().reset(new SimpleMinMaxHeapNode(key, node));
    node = node->left();
  } else {
    node->mutable_right().reset(new SimpleMinMaxHeapNode(key, node));
    node = node->right();
  }
  SimpleMinMaxHeapify(node);
}

int SimpleMinMaxHeap::max() const {
  if (size_ <= 1) {
    return root_->key();
  } else if (size_ == 2) {
...
int SimpleMinMaxHeap::min() const {
  return root_->key();
}

void SimpleMinMaxHeap::pop_max() {
  if (size_ <= 1) {
    size_ = 0;
    root_.reset();
    return;
...
bool SimpleMinMaxHeap::empty() const {
  return size_ == 0;
}

void SimpleMinMaxHeapNode::Print(std::string left_padding) const {
...
void SimpleMinMaxHeap::Print() const {
  if (root_ == NULL) {
    std::cout << "||" << std::endl;
    return;
  }
...
void SimpleMinMaxHeap::SimpleMinMaxHeapify(SimpleMinMaxHeapNode* node) {
...
static SimpleMinMaxHeapNode* Grandparent(SimpleMinMaxHeapNode* node) {
...
void SimpleMinMaxHeap::MaxBubbleUp(SimpleMinMaxHeapNode* node) {
  if (node->parent() == NULL) {
    return;
  }
...
void SimpleMinMaxHeap::MinBubbleUp(SimpleMinMaxHeapNode* node) {
  if (node->parent() == NULL) {
    return;
  }
...
static bool HasGrandchildren(SimpleMinMaxHeapNode* node) {
  return (node != NULL && node->left() != NULL && node->left()->left() != NULL);
}

static SimpleMinMaxHeapNode* CompareNodes(SimpleMinMaxHeapNode* left,
                                          SimpleMinMaxHeapNode* right,
                                          bool want_max) {
  if (right == NULL) {
    return left;
  }
...
static SimpleMinMaxHeapNode* Grandchild(SimpleMinMaxHeapNode* node,
...
void SimpleMinMaxHeap::MaxSiftDown(SimpleMinMaxHeapNode* node) {
  if (node->left() != NULL) {
...
void SimpleMinMaxHeap::MinSiftDown(SimpleMinMaxHeapNode* node) {
  if (node->left() != NULL) {
    if (node->left()->score() < node->score()) {
      Swap(node->left(), node);
    }
...
SimpleMinMaxHeapNode* SimpleMinMaxHeap::GetNode(unsigned node_id) const {
  if (node_id <= 1) {
    return root_.get();
  }
